class Recipes < ActiveRecord::Base
  ...
  validates :cookTime, presence: true, format: {with: /([01]?[0-9]|2[0-3]):[0-5][0-9]/, message: "Cook time can only be a number or a decimal"}
  validates :costOfIngredients, presence: true, format: {with: /[0-9]+\.[0-9][0-9](?:[^0-9]|$)/, message: "Cost of ingredients can only be in the form '39.00'"}
end
